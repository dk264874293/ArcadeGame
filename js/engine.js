...
var Engine = function (global,player) {
    /* 实现定义我们会在这个作用于用到的变量
...
(function(){
    ...
    //配置属性，enemyImg怪物的图片，speed怪物速度，playerImg玩家图片，num怪物数量
    var option = {
        'easy': {
            enemyImg: 'images/Evil-Minion-Icon-3.png',
            speed: 300,
            playerImg: 'images/Edith-despicable-me-2-icon.png',
            num: 6
        },
        'ordinary': {
            enemyImg: 'images/evil-minion-icon-2.png',
            speed: 350,
            playerImg: 'images/happy-agnes-icon.png',
            num: 8
        },
        'hard': {
            enemyImg: 'images/Evil-Minion-Icon-4.png',
            speed: 400,
            playerImg: 'images/girl-minion-icon.png',
            num: 10
        },
    };
    //添加点击事件
    ...
    for(let i = 0 ; i < oLi.length ; i++){
        oLi[i].onclick = function(){
            //获取选项
            var thatInner = this.innerHTML;
            //创建玩家
            var player = new Player(option[thatInner]);
            //影藏选择宽
            select.style.display = 'none';
            //循环添加虫子
            for (let i = 0; i < option[thatInner].num; i++) {
                var enemy = new Enemy(option[thatInner]);
